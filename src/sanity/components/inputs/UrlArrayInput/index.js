...
export function UrlArrayInput(props) {
  ...
  const handleSubmit = () => {
    ...
    if (!isValidUrl) {
      ...
      const alreadyPresent = normalizedValue.includes(urlWithProtocol)
      if (alreadyPresent) {
        setInputValue('')
        return
      }
      const next = [...normalizedValue, urlWithProtocol]
  ...
  return (
    <Stack space={3}>
      <TextInput
        ...
        placeholder={`Add ${schemaType?.title?.toLowerCase() || 'URL'} and press Enter`}
        disabled={readOnly}
        type="url"
      />
